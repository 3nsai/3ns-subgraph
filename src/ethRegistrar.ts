...
const GRACE_PERIOD_SECONDS = BigInt.fromI32(345600); // 4 days
...
export function handleNameRegistered(event: NameRegisteredEvent): void {
  ...
  let domain = createOrLoadDomain(
    crypto.keccak256(concat(rootNode, label)).toHex(),
  ...
  if (checkValidLabel(labelName)) {
    ...
    domain.name = labelName! + ETH_NODE_TEXT;
...
function setNamePreimage(name: string, label: Bytes, cost: BigInt): void {
  ...
  if (domain.labelName != name) {
    ...
    domain.name = name + ETH_NODE_TEXT;
    domain.save();
  }
